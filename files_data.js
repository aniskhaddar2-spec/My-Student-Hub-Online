const MSH_FILES = {
    "assignments": [
        "assignment1.pdf",
        "TD IOT.pdf",
        "td2_rev1.pdf",
        "TP UML.pdf (1).pdf"
    ],
    "important": [
        "cv.pdf",
        "Environmental Engineering Certificate.png",
        "IA and ML Certificate.png",
        "Java Certificate.png"
    ],
    "python": [
        "python.pdf"
    ],
    "ai": [
        "ai.pdf",
        "Correction_TD4.pdf",
        "IA.pdf",
        "TD 6.pdf",
        "TD 7.pdf",
        "TD1.pdf",
        "TD2.pdf",
        "TD3.pdf",
        "TD4.pdf"
    ],
    "data_analysis": [
        "copie_de_ml.py",
        "cour fouille de donneé.pdf",
        "Exercices d’Analyse et Fouille de Données avec Correction.pdf",
        "Netflix dataset.py",
        "série complète de révision en Analyse et Fouille de Données (Data Mining).pdf",
        "série de révision complète en fouille de données (Data Mining).pdf",
        "TP _ Prédiction des Espèces d’Iris avec Machine Learning.pdf",
        "tp fouille de donneé.pdf",
        "TP – Prédiction du prix des maisons (Regression).pdf",
        "Penguins Dataset/penguins_dataset (1).py",
        "Penguins Dataset/penguins_size.csv",
        "Student Performance ML/student performances factors.pbix",
        "Student Performance ML/student_performance.py",
        "Student Performance ML/StudentPerformanceFactors.csv"
    ],
    "math": [
        "Chaitre 2 Variable statistique discrète.pdf",
        "chapitre 3 variable statistique continue.pdf",
        "Chapitre 4 Etude d'une variable statistique à deux dimensions(cours et td corrigé).pdf",
        "charitre 1 Généralité sur la statistique.pdf",
        "correction TD1.pdf",
        "TD3(variable statistique continue).pdf"
    ],
    "english": [
        "Future perfect lesson.pdf",
        "Past progressive-2.pdf",
        "past_simple_or_past_continuous_2-2.pdf",
        "phrasal verbs exercises.pdf",
        "phrasal verbs.pdf",
        "the-future-perfect-and-the-future-simple.pdf"
    ],
    "web_programming": [
        "JF_4_1_fr.pdf",
        "JF_4_2_fr.pdf",
        "JF_4_3_fr.pdf",
        "JF_4_4_fr.pdf",
        "mini_projet_1.pdf",
        "mini_projet_2.pdf",
        "programmation_web1_Partie1.pdf",
        "programmation_web1_Partie2.pdf",
        "programmation_web2_Partie1.pdf",
        "TP1 (1).pdf",
        "TP1.pdf",
        "TP1_Correction.pdf",
        "TP2 (1).pdf",
        "TP2.pdf",
        "TP2_Correction.pdf",
        "TP3 (1).pdf",
        "TP3.pdf",
        "TP3_Correction (1).pdf",
        "TP3_Correction.pdf",
        "TP4 (1).pdf",
        "TP4.pdf",
        "TP4_Correction.pdf",
        "TP5.pdf"
    ],
    "database_systems": [
        "Correction – TP 1.pdf",
        "SGBD.pdf",
        "TP1.pdf",
        "TP2.pdf"
    ],
    "it_fundamentals": [
        "Chap2   cas utilisation.pdf",
        "Chap4 séquence (1).pdf",
        "Chapitre 1 Introduction à la conception orientée objet des systèmes d’information.pdf",
        "chapitre 3 Diagrammes de classe.pdf",
        "chapitre 4diagramme-d-objet.pdf",
        "revision pour examen  UML.pdf",
        "revision pour examen exercices corrigés.pdf",
        "TD _ Langage de modélisation (UML).pdf",
        "Td corrigé diagramme de classe.pdf",
        "TD1 (1).pdf",
        "TP UML.pdf (1).pdf",
        "uml CAS D'UTILISATION CORRECTION.pdf"
    ]
};